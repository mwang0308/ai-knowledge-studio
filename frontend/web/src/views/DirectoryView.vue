    ...
    <section class="panel">
      <div class="panel-head knowledge-panel-head">
        ...
        <div class="btns knowledge-filter-actions">
          ...
          <span class="status blue">共 {{ total }} 个目录</span>
        </div>
      </div>
      <div class="panel-body">
        <table class="table">
          ...
          <tbody>
            <tr v-for="item in pageRows" :key="item.id" :class="{ active: item.id === activeId }">
              <td>
              ...
              <td>
                <span class="ops">
                  <RouterLink class="link" :to="{ path: '/upload', query: { knowledgeBaseId: item.knowledgeBaseId, directoryId: item.id } }">上传文档</RouterLink>
                  <RouterLink class="link" :to="{ path: '/document', query: { knowledgeBaseId: item.knowledgeBaseId, directoryId: item.id } }">文档列表</RouterLink>
...
const query = reactive<{ pageNo: number; pageSize: number; knowledgeBaseId?: number; description?: string; status?: number }>({
  pageNo: 1,
  pageSize: 10,
...
const pageRows = computed(() => {
  ...
  return flatRows.value.slice(start, start + query.pageSize);
});
...
function handlePageSizeChange() {
  query.pageNo = 1;
  activeId.value = pageRows.value[0]?.id;
}
...
async function loadDirectories() {
  ...
  total.value = flatRows.value.length;
