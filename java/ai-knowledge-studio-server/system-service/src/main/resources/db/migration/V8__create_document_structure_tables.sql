create table if not exists knowledge_document_structure
(
    id                    bigint        not null auto_increment comment '主键 ID',
    section_uid           varchar(32)   not null comment '目录节点业务 ID',
    parent_section_uid    varchar(32)   null comment '父目录节点业务 ID',
    ...
    title_path            varchar(1024) not null comment '目录标题路径',
    ...
    primary key (id),
    key idx_doc_structure_section (section_uid, deleted),
    key idx_doc_structure_version (document_id, document_version_id, deleted),
    key idx_doc_structure_parent (parent_section_uid, deleted)
) engine = InnoDB
